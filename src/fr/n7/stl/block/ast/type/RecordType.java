...
public class RecordType implements Type, Declaration, Scope<FieldDeclaration> {
	...
	public boolean equalsTo(Type _other) {
		if(_other instanceof RecordType){
			return ((RecordType)_other).getName().equals(this.name);
	...
	public boolean compatibleWith(Type _other) {
		if(_other instanceof RecordType){
			if(((RecordType)_other).getName().equals(this.name)){
				return true;
			}
		}
		return false;
	...
	public boolean resolve(HierarchicalScope<Declaration> _scope) {
		...
		for (FieldDeclaration f : this.fields) {
			_result =  f.getType().resolve(_scope) && _result;
		}
		return _result;
	}
