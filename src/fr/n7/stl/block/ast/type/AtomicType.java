...
public enum AtomicType implements Type {
	...
	public boolean compatibleWith(Type _other) {
		if (_other instanceof NamedType) {
			if (this.equalsTo(((NamedType)_other).getType())) {
				return true;
			} else {
				switch (this) {
				case NullType : return ((((NamedType)_other).getType() != ErrorType) && (((NamedType)_other).getType() != VoidType));
				case IntegerType: return (((NamedType)_other).getType() == FloatingType);
