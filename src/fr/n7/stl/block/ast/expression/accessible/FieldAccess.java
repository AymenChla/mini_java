...
public class FieldAccess extends AbstractField implements Expression {
	...
	public Fragment getCode(TAMFactory _factory) {
		...
		frag.add(_factory.createPop(0, this.record.getType().length() - this.field.getOffset() - this.field.getType().length()));
		
		frag.add(_factory.createPop(this.field.getType().length(), this.field.getOffset()));
